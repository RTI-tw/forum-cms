...
此項已自 active public findings 移出。原始問題只在 GraphQL/list mutation 可被 public client 直接呼叫時成立；若 production 已強制 GraphQL 只接受 ingress/internal service traffic，public attacker 沒有可達 entry point。另目前程式碼已在非 CMS create 分支忽略用戶端傳入的 `member.connect`，改以 `getOfficialMemberIdForSessionUser(context)` 覆寫；需注意該 helper 是 Keystone CMS User -> Official Member mapping，不是前台 bearer token member identity。
...
此項已自 active public findings 移出。原始問題只在 GraphQL/list mutation 可被 public client 直接呼叫時成立；若 production 已強制 GraphQL 只接受 ingress/internal service traffic，public attacker 沒有可達 entry point。另目前程式碼已在非 CMS create 分支忽略用戶端傳入的 `author` 與 `status`，改以 `getOfficialMemberIdForSessionUser(context)` 覆寫作者並強制 `pending`；該 helper 是 Keystone CMS User -> Official Member mapping，不是前台 bearer token member identity。
