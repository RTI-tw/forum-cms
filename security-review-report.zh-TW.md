...
- 2026-06-09 修正校準：production 仍應強制 `/api/graphql` 只接受 ingress/internal service traffic；同時，非 CMS write path 仍需用 bearer token member identity 綁定前台會員，CMS path 才使用 OfficialMapping。
...
摘要：原始審查共列出 26 個 findings；2026-06-09 修正校準後，`[1]`、`[2]`、`[3]`、`[11]`、`[12]` 仍以程式碼修正保留 defense-in-depth，GraphQL internal-only 則作為額外部署邊界。未找到 first-party SQL injection、runtime user-controlled SSRF、first-party XXE parser 或 first-party prototype-pollution merge path；相關殘餘風險已納入下方 dependency 與 supply-chain findings。

### [1] （已修正）開放 API 的 createComment 信任用戶端提供的 member 關聯
...
此項已修正。非 CMS create 分支會忽略用戶端傳入的 `member.connect`，改以 `getAuthenticatedMemberId(context)` 從前台 bearer token 綁定 `member`；CMS 分支才在未明確指定 `member` 時嘗試 `getOfficialMemberIdForSessionUser(context)`。
...
### [2] （已修正）PollVote mutation 可破壞投票與彙總票數
...
此項已修正。非 CMS create 分支會驗證 poll 可見性、option 歸屬與每位會員每個投票只能投一票，並以 bearer token member 覆寫 `data.member`；非 CMS update/delete 保留 owner filter。
...
### [3] （已修正）Report create/update 對外開放時可隱藏任意文章與留言
...
此項已修正。非 CMS create 只允許前台會員建立 `pending` report，並由伺服器以 bearer token 綁定 `reporter`；非 CMS update/delete 維持 CMS-only。Report 仍保留 create 時 post/comment 擇一的資料完整性檢查與 resolved 狀態同步副作用。
...
`afterOperation` 在 report 變成 `resolved` 時會把 `post` 或 `comment` 設為 `hidden`。前台 create path 會強制 `status = pending`；只有 CMS update 可進行 resolved/dismissed 審核狀態轉換。

#### Dataflow 資料流

CMS 呼叫者 update Report 為 `status=resolved` -> hook 執行 -> 目標內容狀態變成 hidden。非 CMS create 即使送入 `status=resolved` 也會被覆寫為 `pending`。
...
維持公開檢舉提交與審核 resolution 分流：非 CMS create 只能建立 pending report，伺服器端把狀態改為 `resolved` 必須要求 moderator/admin 權限。
...
### [11] （已修正）開放 API 的 createPost 信任用戶端提供的 author 與 status
...
此項已修正。非 CMS create 分支會忽略用戶端傳入的 `author.connect` 與 `status`，改以 `getAuthenticatedMemberId(context)` 從前台 bearer token 綁定 `author`，且一律設定 `status = 'pending'`；CMS 分支才使用 OfficialMapping 自動帶入與 CMS 預設狀態。
...
### [12] （已修正）Bookmark mutation 缺少擁有者隔離
...
此項已修正。Bookmark 非 CMS query/update/delete 都以 bearer token member 加上 owner filter；非 CMS create 會覆寫 `data.member` 為已驗證會員。
...
會員 session 驗證只檢查 member row 是否存在，但不會對既有 bearer token 拒絕 banned/deleted 狀態；`inactive` 則是首次註冊後補 profile 的中間狀態，不應被此查詢擋掉。
...
有效會員 session token -> member status 改為 banned/deleted -> `verifyMemberSession` 成功 -> member id 仍可供 visibility filters 與 mutation 使用。
