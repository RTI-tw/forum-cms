...
部署時若使用 **`ACCESS_CONTROL_STRATEGY=api`**（依 list 限制讀寫，取代 `gql` 全開），請一併設定 **`ACCESS_CONTROL_API_RULES_JSON`** 等變數；完整說明與範例見 [packages/forum-cms/README.md — Access control](./packages/forum-cms/README.md#access-control)。
