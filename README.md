...
部署時若使用 **`ACCESS_CONTROL_STRATEGY=api`**（依 list 限制讀寫，取代 `gql` 全開），請一併設定 **`ACCESS_CONTROL_API_RULES_JSON`** 等變數，並在 **GCP** 以 **IAM／IAP／網路隔離**等方式保護 GraphQL endpoint；完整說明與範例見 [packages/forum-cms/README.md — Access control](./packages/forum-cms/README.md#access-control)。
...
2. 呼叫 `authenticateMemberWithFirebase` 取得會員資料與後端 session token（首次註冊預設為 `inactive`）。
...
- 註冊狀態：首次建立時，Member `status` 預設為 `inactive`；完成 profile 後可透過 `updateMember` 更新為 `active`。
