 ...
 * Keystone 實體讀取與翻譯欄位寫回（word → word_zh … word_th、notice → notice_zh … notice_th 等）。
 */
...
/**
 * Post / Comment / Topic / Poll / PollOption / Content / ForbiddenKeyword / Event 建立或原文變更後，
...
export function createMessageServicesTranslationHook(
  ...
  return async ({ item, originalItem, operation, resolvedData }) => {
    ...
    if (entityType === 'comment' && readMergedBool(rec, orig, 'pauseAutoTranslation')) {
      return
    }
